...
import type { ModuleRunnerTransport, ModuleRunnerTransportHandlers } from "vite/module-runner"
...
export interface BareViteTransportOptions {
  ...
  /**
   * Creates the transport socket. Primarily useful for alternate Bare socket implementations and
   * deterministic tests.
   *
  ...
  /**
   * Observes each decoded Vite payload before it reaches `ModuleRunner`. Returning a promise does
   * not delay delivery to `ModuleRunner`.
   */
...
export function createBareViteTransport(options: BareViteTransportOptions): ModuleRunnerTransport {
