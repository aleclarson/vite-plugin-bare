...
export function dispose() {
  globalThis.__bareViteDeviceReport?.({ type: 'dispose', generation })
}

if (import.meta.hot) {
  import.meta.hot.accept('./value.ts', (next) => {
    globalThis.__bareViteDeviceReport?.({ type: 'hmr', value: next.value })
  })
}

declare global {
  var Bare: { platform: string; arch: string }
